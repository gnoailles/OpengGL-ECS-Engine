...
namespace Context
{
	...
	void Window::Run()
	{
		m_camera->GetComponent<Physics::Transform>()->SetPosition(glm::vec3(0.f, 2.f, -1.f));
		m_camera->GetComponent<Physics::Transform>()->LookAt(glm::vec3(0.0f, 0.0f, 10.0f));
		m_camera->UseOrtho(-2, 2, -1.5, 1.5, 1, 20);

//		m_camera->UsePerspective(90, (16.0f / 9.0f), 1, 10);
		...
		cube.GetComponent<Physics::Transform>()->SetPosition(-1.0f, 0.0f, 10.0f);


		cube2.GetComponent<Physics::Transform>()->SetPosition(1.0f, 0.f, 10.f);
		...
		while (!glfwWindowShouldClose(m_window))
		{
			...
			while (Timer::accumulator >= Timer::fixedDeltaTime)
			{
				//FixedUpdate()
				Timer::FixedUpdate();
				cube.GetComponent<Physics::Transform>()->Rotate(0.0f, 1.0f, 0.0f);
				cube2.GetComponent<Physics::Transform>()->Rotate(1.0f, 0.0f, 0.0f);
			}
