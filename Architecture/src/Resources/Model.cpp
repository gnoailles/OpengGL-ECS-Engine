...
const std::regex Model::VERTEX_PATTERN("^v\\s+([\\d|\\.|\\+|\\-|e|E]+)\\s+([\\d|\\.|\\+|\\-|e|E]+)\\s+([\\d|\\.|\\+|\\-|e|E]+)");
const std::regex Model::NORMAL_PATTERN("^vn\\s+([\\d|\\.|\\+|\\-|e|E]+)\\s+([\\d|\\.|\\+|\\-|e|E]+)\\s+([\\d|\\.|\\+|\\-|e|E]+)");
const std::regex Model::UV_PATTERN("^vt\\s+([\\d|\\.|\\+|\\-|e|E]+)\\s+([\\d|\\.|\\+|\\-|e|E]+)");
const std::regex Model::FACE_VERTEX_PATTERN("^f\\s+(-?\\d+)\\s+(-?\\d+)\\s+(-?\\d+)(?:\\s+(-?\\d+))?");
const std::regex Model::FACE_VERTEX_UV_PATTERN("^f\\s+(-?\\d+)\\/(-?\\d+)\\s+(-?\\d+)\\/(-?\\d+)\\s+(-?\\d+)\\/(-?\\d+)(?:\\s+(-?\\d+)\\/(-?\\d+))?");
const std::regex Model::FACE_VERTEX_UV_NORMAL_PATTERN("^f\\s+(-?\\d+)\\/(-?\\d+)\\/(-?\\d+)\\s+(-?\\d+)\\/(-?\\d+)\\/(-?\\d+)\\s+(-?\\d+)\\/(-?\\d+)\\/(-?\\d+)(?:\\s+(-?\\d+)\\/(-?\\d+)\\/(-?\\d+))?");
const std::regex Model::FACE_VERTEX_NORMAL_PATTERN("^f\\s+(-?\\d+)\\/\\/(-?\\d+)\\s+(-?\\d+)\\/\\/(-?\\d+)\\s+(-?\\d+)\\/\\/(-?\\d+)(?:\\s+(-?\\d+)\\/\\/(-?\\d+))?");
const std::regex Model::OBJECT_PATTERN("^[og]\\s*(.+)?");
const std::regex Model::SMOOTHING_PATTERN("^s\\s+(\\d+|on|off)");
const std::regex Model::MTL_LIB_PATTERN("^mtllib ");
const std::regex Model::MTL_USE_PATTERN("^usemtl ");
...
void Model::InitBuffers(VertexArray& p_va,VertexBufferLayout& p_layout)
{
...
bool Model::ParseOBJ(const std::string& p_filePath)
{
	try
	{
		if (p_filePath.substr(p_filePath.size() - 4, 4) == ".obj")
		{
			...
			else
			{
				...
				LOG(Core::Debug::Log::Error(), logmsg);
				return false;
			}
		}
		else
...
const Buffers::IndexBuffer& Model::GetIndexBuffer() const
{
	return m_ib;
}

const Buffers::VertexBuffer& Model::GetVertexBuffer() const
{
	return m_vb;
}

void Model::GenFace(std::vector<Vertex>& o_faceVertices,
...
void Model::TriangulateFace(std::vector<unsigned>& o_indices, 
...
bool Model::IsInsideTriangle(const vec3f& p_point, const vec3f& p_triangle1, const vec3f& p_triangle2, const vec3f& p_triangle3) const
{
	// Starting vars
...
template <class T>
const T& Model::GetElement(const std::vector<T>& p_elements, const std::string& p_index)
{
