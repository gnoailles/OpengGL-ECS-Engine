...
ShaderProgramSource Shader::Parse(const std::string&& p_filepath)
{
	...
	const GLsizei vtStartIndex = (GLsizei)(shader.find(vertexShaderStart) + vertexShaderStart.size());
	const GLsizei fragStartIndex = (GLsizei)shader.find(fragmentShaderStart);
...
void Shader::CatchCompilerError(const GLenum p_compilerAction, const GLuint p_compiledShader)
{
	...
	if (success == GL_FALSE)
	{
		...
		LOG(Core::Debug::Log::Error(), "Shader " << ((p_compilerAction == GL_COMPILE_STATUS) ? "Compilation" : "Linking") <<
			" failed : " << err);
	}
}
...
int Shader::GetUniformLocation(const std::string&& p_name)
{
	...
	if (location == -1)
		LOG(Core::Debug::Log::Warning(), "Uniform " + p_name + " doesn't exist");
